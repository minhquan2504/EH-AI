...
export class DrugCategoryService {
    ...
    /**
     * Tạo mới nhóm thuốc.
     * Validate: code và name bắt buộc, code phải unique
     */
    static async createCategory(input: CreateDrugCategoryInput): Promise<DrugCategory> {
        if (!input.code || !input.name) {
            throw {
                success: false,
                code: 'PHM_CAT_004',
                message: 'Mã nhóm thuốc (code) và tên nhóm thuốc (name) là bắt buộc.'
            };
        }
     ...
     * Chặn xóa nếu nhóm đang chứa thuốc để đảm bảo toàn vẹn dữ liệu.
     */
     ...
     * Logic upsert: nếu code đã tồn tại → update, chưa có → insert
     */
