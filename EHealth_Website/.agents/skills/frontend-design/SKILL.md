...
Then implement working code (HTML/CSS/JS, React, Vue, Next.js, etc.) that is:
...
Focus on:
- **Typography**: Choose fonts that are beautiful, unique, and interesting. Avoid generic fonts like Arial and Inter; opt instead for distinctive choices that elevate the frontend's aesthetics; unexpected, characterful font choices. Pair a distinctive display font with a refined body font. Use Google Fonts CDN when available.
...
- Use the `generate_image` tool to create custom assets, icons, or illustrations when needed instead of placeholders.
- Use `browser_subagent` to preview and validate the design in a real browser.
- When building React/Next.js apps, use `run_command` to scaffold and run dev servers.
